 ...
 * 
 * 1. 사용자가(클라이언트) URL을 클릭하면 HttpRequest 를 Servlet Container로 전송(요청)한다.
 * 2. 컨테이너는 web.xml에 정의된 URL패턴을 확인하여 어느 서블릿을 통해서 처리할 지를 검색한다.
 * 		(해당 서블릿이 로딩이 안된 경우에는 로딩을 한다. ==> 로딩시 init() 메서드가 호출된다.)
 * 		(Servlet 3.0 이상에서는 어노테이션(@WebServlet)으로 설정이 가능하다.)
 * 3. Servlet Container 는 요청을 처리할 때 개별 쓰레드 객체를 생성하여 해당 서브릿 객체의 service()메서드를 호출한다.
