...
/*
 * @WebServlet 어노테이션은 Servlet 버전 3.0 이상엥서 사용할 수 있다.
 * 
 * @WebServlet 어노테이션의 속성들
 * 1. name : 서블릿의 이름을 설정한다. (기본값 : 빈 문자열(""))
 * 2. value : 서블릿의 URL패턴을 설정한다.
 * 3. urlPatterns : 서블릿의 URL 패턴의 목록을 설정한다. (기본값 : 빈배열( {} ))
...
public class ServletTest02 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		res.setCharacterEncoding("UTF-8");	//응답문서의 인코딩 방식 지정
		res.setContentType("text/html; charset=utf-8"); 	//응답문서의 ContentType 지정
		System.out.println("request : " + req + " / response : " + res);
		//처리한 내용을 응답으로 보내기 위해서 PrintWriter 객체 생성한다.
