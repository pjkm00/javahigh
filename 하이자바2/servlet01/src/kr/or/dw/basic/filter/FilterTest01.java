 ...
 * 		- 클라이언트에 대한 응답을 정보를 제공하기 전에 응답정보에 필요한 작업을 수행할 수 있다.
 * 
 * 2. 사용예
 * 		- 인증 작업용 필터
 * 		- 데이터 압축용 필터
...
public class FilterTest01 implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		System.out.println("첫번째 Filter - doFilter() 메서드 호출");
		
		chain.doFilter(request, response); 	//다음필터로 체이닝하는 메서드(다른필터로 제어권을 넘겨주는 명령어)
