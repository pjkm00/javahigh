...
public class SessionReadServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		out.println("<body>");
		out.println("<h2>저장된 Session 데이터가 확인하기</h2><br><br>");
		...
		//		==>세션에 저장된 모든 '세션key값'을 구해서 Enumeration 객체로 변환한다.(순서를 부여해주는 객체)
		...
		//생성시간 ==> 1970년 1월 1일부터 경과한 시간을 밀리세컨드단위로 반환
		out.println("세션 생성 시간 : " + session.getCreationTime() + "<br>");
		//최근접근시간 ==> 1970년 1월 1일부터 경과한 시간을 밀리세컨드단위로 반환
		out.println("세션 최근 접근 시간 : " + session.getLastAccessedTime() + "<br>");
		//세션 유효 시간 ==> 초단위
		//세션의 최근 접근 시간부터 세션 유효시간이 경과하면 세션이 자동으로 삭제
