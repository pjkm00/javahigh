...
public class ServletTest04 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		/*
		 * Servlet 클래스나 JSP페이지의 환경에 관한 정보는 ServletContext 인터페이스 타입의 객체를 이용해 구할 수 있다.
		 */
		ServletContext context = this.getServletContext();
		String serverInfo = context.getServerInfo(); 	//Servlet이 속한 웹 서버의 종류 구하기
		
		//웹 컨테이너가 지원하는 Servlet 규격서의 메이저버전과 마이너버전 구하기
		 ...
		 * 특성 Servlet에 속하는 초기화 파라미터가 아닌 웹 어플리케이션 전체에 속하는 초기화 파라미터이다.
		 * 
		 * 이 초기화 파라미터는 web.xml 파일이 속한 웹 어플리케이션 디렉토리의 모든 웹 컴포넌트에서 읽기가 가능하다.
		 * 
		 * Servlet 클래스에는 getServletContext() 메서드를 통해 ServletContext 객체를 구하고,
