...
public class CookieAddServlet extends HttpServlet{
	...
	//- 세션 쿠키 vs 영구적 쿠키
	//1. 만료시각을 지정하면 영구적 쿠키, 파일저장
	//2. 만료시각이 없으면 세션쿠키, 메모리에 저장, 브라우저 닫으면 삭제됨
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		//		==> '쿠키값'으로 한글을 사용할 경우 URLEncorder.encode()메서드로 인코딩 후 사용한다.
		// 쿠키는 쿠키를 생성한 서버에만 전송된다. ==> 다른 사이트로 연결할 떄에는 전송되지 않는다.
		// 한 도메인당 20개, 쿠키하나당 4KB
		...
		//2. response 객체를 이용하여 쿠키를 웹 브라우저로 보낸다.
