...
public class CookieDeleteServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		//저장된 쿠키 삭제하기
		//==> 쿠키데이터의 삭제는 쿠키의 유지시간을 0으로 설정하는 방법을 사용한다.
		//형식) cookie객체변수.setMaxAge(시간);		//시간 밀리세컨드(1/1000초) 단위로 지정한다.
		//		==> 시간이 0이면 쿠키가 바로 삭제되고
		//		==> 시간이 음수이면 웹브라우저가 끝날때 쿠키가 삭제된다.
		//		==> 시간이 양수이면 지정된 시간동안 저장된 후 지정된 시간이 경과하면 자동으로 삭제된다.
		...
		out.println("<a href='" + req.getContextPath() + "/basic/02/cookieTest.jsp'>시작화면으로 돌아가기</a>");
