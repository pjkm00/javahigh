...
public class JdbcTest01 {
	//JDBC(Java DataBase Connectivity) 라이브러리를 이용한 DB자료 처리하기
		 ...
		 *  Class.fotName("oracle.jdbc.driver.OracleDriver");
		 ...
		 *  1) SQL문이 SELECT문일 경우 : select한 결과가 ResultSet 객체에 저장되어 반환된다.
		 *  2) SQL문이 SELECT문이 아닐경우(INSERT, UPDATE, DELETE, CREATE 등등)
	...
	public static void main(String[] args) {
		...
		try {
			...
			//3. SQL문 작성 : SQL문을 작성하는 부분. SQL은 대소문자를 구분하지 않는다.
			String sql = "select * from emp";
			
			//4. Statement 객체생성 : 질의를 수행하는 객체 생성
			...
			//7. 사용했던 자원 반납
			//close 하기 전에는 메모리 상에 파일을 쓰기 위한 정보를 넣어놓는거고,
			//close 하는 시점에서 쓰기 처리가 되는것.
